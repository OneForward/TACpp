...
### 使用 `printf`
`printf` 中格式化字符串的形式如下，其具体规则见 [`printf` 详解](#printf-详解) 

`%[flags][width][.precision][length]specifier`

* 例如 `%-10g` 代表将浮点数字格式化为宽度为10，左对齐的字符串。

* 例如 `%010.3f` 代表将浮点数字格式化为宽度为10，右对齐，前缀填充了0的字符串。
...
2.  `printf` 相对而言字符串更紧凑，易于理解。而`std::cout` 的流控制语义非常复杂，且有滥用 流运算符 `<<` 的趋势，因此不易理解。
...
4. `printf` 最为人诟病的是类型不安全。不过可以设置编译器提供编译期间的检查，并产生 Warning。
